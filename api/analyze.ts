...
async function readUploadedImage(req: VercelRequest): Promise<{ buffer: Buffer; mimeType: string } | null> {
  const bb = Busboy({ headers: req.headers as Record<string, string>, limits: { fileSize: MAX_IMAGE_BYTES } });
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    // Vercel이 body를 이미 Buffer로 읽어둔 경우와, 스트림이 아직 살아있는 경우를 모두 처리한다.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!image) {
      res.status(400).json({ status: 'error', message: '이미지 파일을 확인해주세요 (jpeg/png/webp, 10MB 이하)' });
      return;
    }
    ...
    if (!parsed || !isAnalyzedItem(parsed)) {
      res.status(422).json({ status: 'error', message: '이미지에서 제품을 인식하지 못했어요' });
      return;
    }
    ...
    res.status(500).json({ status: 'error', message: '이미지 분석 중 오류가 발생했어요. 잠시 후 다시 시도해주세요.' });
  }
}
